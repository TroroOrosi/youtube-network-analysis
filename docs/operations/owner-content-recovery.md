...
2026-09-17の追加機能です。`--include-subscribers` をPLANとAPPLYの両方に指定すると、
動画・コメントの置換ジョブに加え、登録者用の新規`SUBSCRIBERS`を一件だけ追加します。
既存の完了済み登録者ジョブ・登録者データ・利用量・権限は変更しません。
旧ツールですでに動画・コメントを再受付済みなら、登録者用だけを追加します。
途中再開位置を持たない完了済み取得は、1,001件目の位置を捏造せず最初のページから新規取得します。

新規登録者の同一決定的IDが存在する場合は、成功・失敗・実行中のどれでも再追加しません。
別のアクティブな登録者収集、または元の障害ジョブより新しい登録者収集がある場合は停止します。
両種類の計画全体が検証に通ってから一つのFirestoreトランザクションで保存します。
`subscriber_run_id` と `subscriber_status` が登録者側の識別子・状態です。

**YouTube側の件数制限は撤廃されません。** `mySubscribers`・全段階の日跨ぎ継続を含む
更新後のランタイムをCloud Runへ反映してから利用してください。コードは
[日跨ぎ収集の運用事項](multiday-collection.md)を参照します。

```bash
python3 recover_owner_content.py --target target.json --include-subscribers
# 以下は停止・バックアップ・更新後コードの反映確認後だけ実行する。
python3 recover_owner_content.py --target target.json --include-subscribers \
  --apply --expected-revision "$VERIFIED_REVISION" \
  --writers-stopped --patched-code-verified
```

確認のPLANにも同じ`--include-subscribers`を指定してください。デフォルトは従来どおり動画・コメントのみです。
...
4. **PR #9のページ再開・重複行対策・保存対策と、PR #10追加の日跨ぎ収集コードを本番に反映済みであることを確認します。** ソースのマージだけでは不十分です。未反映なら、先に [保存形式移行の運用手順](2026-09-16-availability-large-channels.md) の整合バックアップ・旧新ライター切替手順でデプロイし、その後もう一度停止状態を確認します。旧版のまま再受付しません。新旧リビジョンが同じDBに書くカナリア展開は禁止です。このツールのjobs単体バックアップは、PR #9の保存形式移行用の全体バックアップの代用ではありません。
