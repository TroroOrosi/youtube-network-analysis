...
本番Web UIの通常の「視聴者ネットワーク分析」は、**接続チャンネルのライブ収集結果**
だけを表示する。動画コメント投稿者を視聴者の代理とし、その投稿者が公開している
登録チャンネルを YouTube Data API で別ジョブとして収集する。非公開の登録関係は
取得不可として記録し、推測・補完しない。ページング、実行時間の区切り、日次quota待ちは
既存の収集ジョブと同じ再開機構を使用する。

個人単位の視聴者IDや行レベルの登録関係はWeb応答に出さず、共通購読・登録数分布・
興味カテゴリ・強いネットワーク関係などの集計だけを表示する。ライブ版は外部の
チャンネル総登録者数を追加取得しないため、規模補正liftは表示しない。

従来の匿名化済み2026-06-22調査データは、ライブ結果と混同しない
**明示的な参考デモ**（`/audience-network?demo=true`）として残す。参考デモのExcelは
次のコマンドでWeb用JSONと同時に再生成できる。
...
- Googleアカウントでログインし、ワークスペースとメンバーを管理する。
- 複数のYouTubeチャンネルを接続・解除し、手動収集と定期収集を実行する。
- 収集の進捗、取得日、対象範囲、API上限、失敗理由と再開方法を確認する。
- 登録時期、公開登録者の状態、コメント活動、セグメントで絞り込み、KPIと一覧を分析する。
- 分析条件を保存し、複数チャンネルを比較する。
- コメント投稿者の公開登録先を別ジョブで収集し、接続チャンネルごとのライブ視聴者
  ネットワークを集計表示する。
- 必要な場合だけ、明示的な参考デモを開き、既存調査データのExcelをダウンロードする。

ライブ収集データと参考調査データは混在させず、参考デモをログイン中のアカウント由来として
表示しない。YouTube APIが公開しない登録関係は推測・補完しない。
