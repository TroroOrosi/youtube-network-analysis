...
本番Web UIの「視聴者ネットワーク分析」は、ログイン中のチャンネルとは分離した
**匿名化済み既存調査データ（デモ）**を表示する。取得日は 2026-06-22 で、個人単位の
視聴者ID、チャンネルID、行レベルの登録関係、認証情報はWeb応答やExcelに含めない。

```bash
python scripts/export_web_audience_report.py
```

このコマンドは同じ集計結果から、Web用JSONとダウンロード用の9シートExcelを同時に
再生成する。WebとExcelには、共通購読上位、登録チャンネル数分布、興味カテゴリ、
コミュニティ、強いネットワーク関係、親和度、人気度と親和度の差を収録する。
...
- データが少ない場合も、上記の匿名化済み調査データでオーディエンスネットワーク分析を
  確認し、同じ集計値のExcelをダウンロードする。

ライブ収集データと調査データは混在させず、調査データをログイン中のアカウント由来として
