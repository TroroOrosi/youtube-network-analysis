...
- [ ] Immutable/slotted run, schedule, quota, page, and command values with
  stable enums and the nine safe error codes.
- [ ] Boundary validation rejects unbounded identifiers, naive datetimes,
  non-positive quota, intervals under one hour, and out-of-range page limits.
- [ ] Ports require `WorkspaceContext` on every tenant operation and depend
  only on the `channel-connections` broker and `channel-data` writer protocols.

**Verification:**

- [ ] RED then GREEN: `python -m unittest collection_jobs.tests.test_models -v`
- [ ] Existing suites, compile, and `git diff --check` pass.

## Task 2: Execute a subscriber run end to end

**Acceptance criteria:**

- [ ] `enqueue_run` requires `collection.run`, rejects a second active run for
  the same connection and kind, and is idempotent per key.
- [ ] `execute_run` issues a fresh execution authority, pages the broker,
  publishes one snapshot, finishes the `channel-data` collection `COMPLETE`,
  and reports pages and quota spent.
- [ ] A `RUNNING` run cannot be executed twice and invalid transitions fail
  without mutation.

**Verification:**

- [ ] RED then GREEN: `python -m unittest collection_jobs.tests.test_runs -v`
- [ ] Full suites, compile, and integrity checks pass.

## Task 3: Enforce quota, retries, cancellation, and fail-closed reauth

**Acceptance criteria:**

- [ ] A run stops before an unaffordable call, finishes `PARTIAL` with
  `QUOTA_EXHAUSTED`, and finishes its `channel-data` collection `PARTIAL`.
- [ ] Transient failures requeue with exactly 1, 5, and 25 minute backoff and
  fail after three attempts; quota and reauth failures never auto-retry.
- [ ] A revoked grant fails the run as `REAUTH_REQUIRED` and publishes nothing.
- [ ] `cancel_run` stops a queued or running run and records `CANCELLED`.

**Verification:**

- [ ] RED then GREEN: `python -m unittest collection_jobs.tests.test_policy -v`
- [ ] Full suites, compile, and integrity checks pass.
...
- [ ] An `OWNER_CONTENT` run publishes the video inventory, finishes it
  `COMPLETE`, then covers every video of that exact inventory, including videos
  with no comments.
- [ ] Coverage failure before completion leaves the previous accepted dataset
  readable and promotes nothing.
- [ ] Stored activity carries only per-video, per-author counts and latest
  times.

**Verification:**

- [ ] RED then GREEN: `python -m unittest collection_jobs.tests.test_content -v`
- [ ] Full suites, compile, and integrity checks pass.
...
- [ ] Schedules require `collection.run`, enforce the one-hour minimum, and
  `enqueue_due_runs` creates at most one active run per connection and kind.
- [ ] Run and schedule reads require `collection.read`, order newest first, and
  use opaque single-use cursors bound to workspace, query, and revision.
- [ ] Missing and foreign runs or schedules share one safe error.

**Verification:**

- [ ] RED then GREEN: `python -m unittest collection_jobs.tests.test_schedules -v`
- [ ] Full suites, compile, and integrity checks pass.
...
- [ ] Identical run, schedule, and connection identifiers in two workspaces stay
  independent across execution, reads, cascade, and quota accounting.
- [ ] Retention removes terminal runs, quota ledger entries, and idempotency
  records at exactly 90 days without purging unresolved runs.
- [ ] An end-to-end fixture drives provider rows into a ready
...
- [ ] Focused and full regression suites, compile, Notebook cells, Markdown
  fences, secret scan, graph review, and `git diff --check` pass.

## Final checkpoint: collection-jobs complete

- [ ] All six tasks are committed and pushed.
- [ ] Every success criterion has direct evidence.
- [ ] No dependency, network call, credential, database, queue, or real data was
  introduced.
