...
A run that vanished on a restart is a run the operator has to guess about: it
may have already spent provider quota, and re-running it spends that quota
twice. The format belongs here, not to the store, which only keeps one text.
